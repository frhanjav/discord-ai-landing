...
const partners = [
  {
    ...
    icon: Mic,
    role: "Converts voice input to text with WebSocket streaming for instant processing",
    placeholder: (
      <img
        src="https://cdn.prod.website-files.com/6458f30fed157c01444bd0b2/66914a5d5cdd9676a68fa3f6_gladia-logo.svg"
        alt="Gladia Logo"
  ...
  {
    name: "OpenAI",
    description: "GPT-5 Nano AI Responses",
    icon: Brain,
    role: "Powers intelligent, context-aware conversations with real-time information",
    placeholder: (
      <svg
  ...
  },
  {
    ...
    icon: Volume2,
    role: "Transforms AI responses into natural, expressive speech for each personality",
    placeholder: (
      <img
        src="https://cdn.prod.website-files.com/66b3765153a8a0c399c70981/66bb1f44d832debb33fe65c3_Logo.svg"
        alt="Murf AI Logo"
        className="h-8 mx-auto"
        style={{ maxWidth: "80px" }}
      />
...
const TechnologyPartners = () => {
  return (
    <section id="technology" className="py-24 px-4 bg-gradient-secondary">
      <div className="container mx-auto">
        <div className="text-center mb-16 animate-fade-up">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
            Built on the foundation of three cutting-edge AI technologies
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {partners.map((partner, index) => (
            <Card
              key={partner.name}
              className={`p-8 bg-card/40 backdrop-blur-glass border border-white/10 hover:border-white/20 hover:shadow-card transition-all duration-500 animate-fade-up ${
                partner.featured ? "ring-2 ring-primary/30 shadow-glow" : ""
              }`}
              style={{ animationDelay: `${index * 0.2}s` }}
            >
              <div className="text-center">
                <div
                  className={`inline-flex items-center justify-center w-16 h-16 rounded-full bg-secondary/30 mb-6 ${partner.accent}`}
                >
                  <partner.icon className="w-8 h-8" />
                </div>

                <div
                  className={`text-lg font-bold mb-2 px-4 py-2 rounded-lg bg-muted/20 border border-dashed border-muted-foreground/30 ${partner.accent}`}
                >
                  {partner.placeholder}
                </div>

                <p className={`font-semibold mb-4 ${partner.accent}`}>
                  {partner.description}
                </p>
        ...
        <div className="text-center mt-16 animate-fade-up">
          ...
          <p className="text-sm text-muted-foreground mt-4">
            Seamless pipeline: Voice Input → AI Processing → Natural Speech
            Output
          </p>
        </div>
      </div>
